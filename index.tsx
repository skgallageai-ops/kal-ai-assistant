...
const App = () => {
  ...
  const [attachments, setAttachments] = useState([]); // Files තබා ගැනීමට
  ...
  // ෆයිල් සිලෙක්ට් කරන කොට ක්‍රියාත්මක වන ෆන්ක්ෂන් එක
  ...
  const handleSend = async () => {
    ...
    try {
      // ඔයාගේ පරණ කෝඩ් එකේ තිබූ මොඩල් එකම පාවිච්චි කර ඇත
      const model = genAI.getGenerativeModel({ model: "gemini-3-flash-preview" }); 
      let promptParts = [input || "මෙම ගොනුව විස්තර කරන්න."];
      
      // ෆයිල්ස් තිබේ නම් ඒවා එකතු කිරීම
